...
function addMessage(content, type, sources = null, isWelcome = false) {
    ...
    if (sources && sources.length > 0) {
        const sourcesHtml = sources.map(s => {
            ...
            return `<span class="source-plain">${escapeHtml(s.name)}</span>`;
        }).join('');
        html += `
...
async function createNewSession() {
    ...
    addMessage('Welcome to the Course Materials AI Assistant! I can help you with questions about courses, lessons and specific content. What would you like to know?', 'assistant', null, true);
}
